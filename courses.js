...
//   First word must match a course name (English, French, Arabic, Spanish, Italian, Dutch, Hebrew)
...
function renderSchedule(events) {
  ...
  container.innerHTML = events.map(event => {
    ...
    // Match to a course by checking if the title includes the course name
    const course = courses.find(c =>
      title.toLowerCase().includes(c.nameEn.toLowerCase()) && !c.isService
    ) || null;

    // Parse level from title: "English - Beginner | Morning" → "Beginner"
    ...
    // Spots bar
    let spotsHTML = "";
    if (course && course.totalSpots !== null) {
      const available = course.totalSpots - course.takenSpots;
      const pct = Math.round((course.takenSpots / course.totalSpots) * 100);
      const spotsColor = available <= 2 ? "#ED2939" : available <= 4 ? "#f59e0b" : "#16a34a";
      spotsHTML = `
        <div class="hidden sm:flex items-center gap-2 min-w-[90px]">
          <div class="flex-1 bg-gray-100 rounded-full h-1.5">
            <div class="h-1.5 rounded-full" style="width:${pct}%;background:${spotsColor}"></div>
          </div>
          <span class="text-xs font-semibold whitespace-nowrap" style="color:${spotsColor}">${available}/${course.totalSpots}</span>
        </div>`;
    }

    const enrollBtn = courseIndex >= 0
      ? `<button onclick="openEnrollModal(${courseIndex})"
    ...
    return `
      <div class="flex items-center gap-4 bg-white rounded-2xl px-5 py-4 shadow-sm border border-gray-100 card-hover">
        <div class="text-2xl shrink-0">${flag}</div>
        <div class="flex-1 min-w-0">
          <div class="font-bold text-gray-900 text-sm">
            <span class="lang-en">${nameEn}${level ? ` – ${level}` : ""}</span>
            <span class="lang-ar">${nameAr}${level ? ` – ${level}` : ""}</span>
          </div>
          <div class="text-gray-400 text-xs mt-0.5">${dateStr}${timeStr ? ` · ${timeStr}` : ""}</div>
        </div>
        ${spotsHTML}
        ${enrollBtn}
      </div>`;
