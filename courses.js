...
const courses = [
  {
    ...
    levels: ["Kids (5-15 yrs)", "Beginner", "Intermediate", "Advanced", "IELTS & TOEFL Prep", "Conversation – Intermediate", "Conversation – Advanced"],
    levelsAr: ["أطفال (5-15 سنة)", "مبتدئ", "متوسط", "متقدم", "تحضير IELTS و TOEFL", "محادثة – متوسط", "محادثة – متقدم"],
  ...
  },
  {
    ...
    levels: ["Kids (5-15 yrs)", "A1/1", "A1/2", "A2/1", "A2/2", "A2/3", "B1/1", "B1/2", "B1/3", "B2/1", "B2/2", "DELF A1 Prep", "DELF A2 Prep", "DELF B1 Prep", "DELF B2 Prep", "DELF C1 Prep", "DELF Scolaire A1", "DELF Scolaire A2", "DELF Scolaire B1", "DELF Scolaire B2", "Conversation – Intermediate", "Conversation – Advanced", "Peacekeeping (French)"],
    levelsAr: ["أطفال (5-15 سنة)", "A1/1", "A1/2", "A2/1", "A2/2", "A2/3", "B1/1", "B1/2", "B1/3", "B2/1", "B2/2", "تحضير DELF A1", "تحضير DELF A2", "تحضير DELF B1", "تحضير DELF B2", "تحضير DELF C1", "DELF Scolaire A1", "DELF Scolaire A2", "DELF Scolaire B1", "DELF Scolaire B2", "محادثة – متوسط", "محادثة – متقدم", "تدريب حفظ السلام (فرنسي)"],
...
async function fetchSpotCounts() {
  ...
  try {
    ...
    // Aggregate multiple sheet rows into one course card.
    // Exact match takes priority; prefix match handles groups
    // e.g. "english-a" + "english-b" both map to course id "english"
    const aggregated = {};
    spots.forEach(({ id, taken, total }) => {
      if (isNaN(taken) || isNaN(total)) return;
      const sheetId = id.toLowerCase();
